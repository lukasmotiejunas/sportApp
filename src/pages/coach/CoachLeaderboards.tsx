...
export default function CoachLeaderboards() {
  ...
  const create = () => {
    ...
    addCategory(draft);
    push({ kind: 'success', message: 'Rezultatų lentelės kategorija sukurta.' });
    setOpen(false);
    setDraft({
      name: '',
      event: '',
    ...
    <div>
          ...
          return (
            <Link key={c.id} to={`/coach/leaderboards/${c.id}`} className="group surface flex flex-col p-4 hover:shadow-card">
              ...
              <div className="mt-auto flex items-center justify-between text-xs text-ink-500">
                ...
                <span className="font-semibold">Geriausias {top ? formatResult(top.value, c) : '—'}</span>
              </div>
              <div className="mt-3 flex flex-wrap gap-1.5">
                <StatusBadge tone="neutral">{c.genderCategory === 'all' ? 'Visi nariai' : c.genderCategory === 'male' ? 'Vyrai' : 'Moterys'}</StatusBadge>
                <StatusBadge tone="info">{c.lowerIsBetter ? 'Mažiau — geriau' : 'Daugiau — geriau'}</StatusBadge>
              </div>
              <button
                type="button"
                onClick={(e) => {
                  e.preventDefault();
                  archive(c.id);
                  push({ kind: 'info', message: 'Rezultatų lentelė archyvuota.' });
                }}
      ...
      {archived.length > 0 && (
        <section className="mt-6">
          <h2 className="mb-2 text-sm font-semibold uppercase tracking-wide text-ink-500">Archyvuota</h2>
      ...
      <Modal
        ...
        footer={
          <>
            <button className="btn-ghost" onClick={() => setOpen(false)}>Atšaukti</button>
            <button className="btn-primary" onClick={create}>Sukurti kategoriją</button>
          </>
        }
      >
        <div className="grid gap-3 sm:grid-cols-2">
          <FormField label="Pavadinimas" required value={draft.name} onChange={(e) => setDraft({ ...draft, name: e.target.value })} />
          <FormField label="Rungtis" required placeholder="pvz. 100 m" value={draft.event} onChange={(e) => setDraft({ ...draft, event: e.target.value })} />
          <SelectField label="Matavimo tipas" value={draft.measurementType} onChange={(e) => {
          ...
          <SelectField label="Kryptis" value={draft.lowerIsBetter ? 'lower' : 'higher'} onChange={(e) => setDraft({ ...draft, lowerIsBetter: e.target.value === 'lower' })}>
            <option value="lower">Mažiau — geriau</option>
            <option value="higher">Daugiau — geriau</option>
          </SelectField>
          <SelectField label="Lyties kategorija" value={draft.genderCategory} onChange={(e) => setDraft({ ...draft, genderCategory: e.target.value as any })}>
