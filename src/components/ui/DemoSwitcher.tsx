...
export function DemoSwitcher({ variant = 'compact' }: Props) {
  ...
  const activeCoach = coaches.find((c) => c.id === currentCoachId) ?? coaches[0];
  ...
  return (
    <>
      ...
      >
        <Avatar
          name={activeName}
          color={role === 'coach' ? activeCoach?.avatarColor : activeMember?.avatarColor}
          size={variant === 'compact' ? 'sm' : 'md'}
        />
        <div className="flex-1 min-w-0 text-left">
          <p className="text-xs uppercase tracking-wide text-ink-500">Demo · {role ?? 'guest'}</p>
          <p className="truncate text-sm font-semibold text-ink-900 dark:text-ink-50">{activeName}</p>
        </div>
                ...
                return (
                  ...
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      <p className="truncate text-sm font-semibold text-ink-900 dark:text-ink-50">{m.name}</p>
                      <p className="truncate text-xs text-ink-500">{m.preferredDistance}</p>
                    </div>
                    <StatusBadge
                      tone={
              ...
              {coaches.map((c) => {
                ...
                return (
                  ...
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      <p className="truncate text-sm font-semibold text-ink-900 dark:text-ink-50">{c.name}</p>
